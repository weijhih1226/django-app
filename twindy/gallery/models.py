...
class BlockType(models.Model):
    type_name = models.CharField("種類名稱" , max_length = 30 , help_text = "必填，僅允許英數字及'-'符號" , blank = False , null = False , validators = [alphanumeric] , unique = True)
    type_dname = models.CharField("種類顯示名稱" , max_length = 30 , blank = True , null = True)
...
class ImageInfo(models.Model):
    data_name = models.CharField("資料名稱" , help_text = "必填" , max_length = 30 , blank = False , null = False , unique = True)
    ...
    block_type = models.ForeignKey(BlockType , verbose_name = "區塊種類" , on_delete = models.CASCADE , blank = True , null = True , related_name = 'image')
...
class ImageBlock(models.Model):
    block_name = models.CharField("區塊名稱" , max_length = 30 , help_text = "必填，僅允許英數字及'-'符號" , blank = False , null = False , validators = [alphanumeric] , unique = True)
    block_dname = models.CharField("區塊顯示名稱" , max_length = 30 , blank = True , null = True)
    image_info = models.ForeignKey(ImageInfo , verbose_name = "圖片資訊" , on_delete = models.CASCADE , blank = True , null = True , related_name = 'image')
